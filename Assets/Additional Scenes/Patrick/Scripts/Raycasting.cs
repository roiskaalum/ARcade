...
public class Raycasting : MonoBehaviour
{
    ...
    [SerializeField] private float moveSmoothness = 10f; //H�jere = mere hastighed
    ...
    // Metode som fors�ger at v�lge en bold.
    ...
    private void MoveBall(Vector2 screenPosition)
    {
        ...
        else
        {
            ...
            // Gemmer bev�gelse som velocity
            velocity = (screenPosition - lastScreenPos) / Time.deltaTime;
            lastScreenPos = screenPosition;

            // Tjekekr for m�ngden af queues i positionHistory til udregning af swipe kraft/retning
    ...
    private void ReleaseBall()
    {
        if (positionHistory.Count >= 2)
        {
            ...
            forward.y = 0f; // Fjern h�ldning i y-retningen
            forward.Normalize();

            // Beregn y-boost baseret p� hvor h�jt bolden blev sluppet
            float yBoost = Mathf.Lerp(0.0f, 0.8f, releaseHeightFactor * 0.5f);

            // Begr�ns sidep�virkning
            ...
            // Boost fremad i z-retning afh�ngig af swipe speed
